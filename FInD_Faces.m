...
clear all;
cd('C:\Users\bexlab\Dropbox\Kerri_Walter\BasalFace')
...
%             trialRecord(pcNum,trialNo).targLevel=logspace(log10(minTestLevel), log10(maxTestLevel), trialRecord(pcNum,trialNo).nTargs); % log spaced range of contrasts between min and max
            ...
            for cellNum=1:nCells % work through each cell
                ...
                ranPCdirs=randsample([-1 1], 199, 1)'; % random increase or decrease where all PCs change
                face_coords=shape_coords;
                
                % render and draw first face
                if targPC(pcNum)==0 % all PCs change
                    face_coords=shape_coords+ranPCdirs*trialRecord(pcNum,trialNo).targLevelPerLoc(cellNum)/2; % set target PC to mid level + threshold deviation
                else % single PC changed
                    face_coords(targPC(pcNum))=centerPCVal(cellNum)+trialRecord(pcNum,trialNo).targLevelPerLoc(cellNum)/2; % set target PC to mid level + threshold deviation                end
                end
                
                mesh1  = coef2object(face_coords, model.shapeMU, model.shapePC, model.shapeEV ); % Convert into vertex space
                ...
                if targPC(pcNum)==0 % all PCs change
                    ...
                    face_coords(targPC(pcNum))=centerPCVal(cellNum)-trialRecord(pcNum,trialNo).targLevelPerLoc(cellNum)/2; % set target PC to mid level + threshold deviation
                end
            ...
            if saveImage
                myImfileName=sprintf('C:\\Users\\Kerri\\Dropbox\\Kerri_Walter\\BasalFace\\%dTrial%d.jpg', pcNum,trialNo);
            ...
            if mod(tcount,length(targPC)) == 0 %break
                Screen('Flip', windowPtr); % clear screen
    ...
    save(dataFile,'trialRecord','expDuration','targPC','threshEsts','faceSize','nRows','nCols','nTrials','scrnWidthCm','viewDistCm','frameRate','winRect');
    ...
    save(dataFile,'trialRecord','thresholdEst','fitLB','fitUB','expDuration','targPC','threshEsts','faceSize','nRows','nCols','nTrials','scrnWidthCm','viewDistCm','frameRate','winRect');
catch
    Screen('CloseAll');
    testSName=sName;    % modify sName if subject already exists
    n=1;
